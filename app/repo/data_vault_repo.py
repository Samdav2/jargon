...
async def get_user_data(user_id: str , db: AsyncSession):
    statement = select(User).where(User.id == user_id).options(selectinload(User.data_vault_entries))
    try:
        ...
        if not data:
            raise HTTPException(detail="User does not have any data saved, pls save data and try again", status_code=404)

    except Exception as e:
        raise HTTPException(detail=f"Error performing this request. Full details {e}", status_code=500)
