...
async def create_data_vault_entry(
    data_vault_create: UserDataVautltCreate,
    db: AsyncSession = Depends(get_session),
    current_user = Depends(get_current_user)
    ):

    data_vault_create.user_id = current_user.id
...
async def data_vic_approve_reject(
    data_id: str, response: Decision,
